...
write machine-owned JSON and environment files that mirror the cloud/control-plane
...
reachable and must not mint secrets.
...
- `server.baseUrl`, `host`, and `port`; default is
  `http://127.0.0.1:3000`.
- `mode`: `demo` or `server`.
- `providerMode`: `demo`, `real`, or `disabled` per provider.
...
```json
{
  "agent": {
    ...
    "externalIdentities": [
      ...
      { "provider": "runtime", "subject": "codex" }
    ]
  },
  "scopes": [
    "tasks:read",
...
seed or ingest one setup verification task through the normal assignment path.
...
should not have to understand four control-plane objects before their first
...
- Interactive setup must be resumable. If `.agentrail/config.json` exists, the
  ...
  equivalent command.
- All server mutations use deterministic idempotency keys such as
  `setup-agent:agt_codex_local:v1`.
...
- The CLI never commits `.agentrail/agent.env`, provider tokens, or generated
  AgentRail API keys.
- The first auth-enabled local bootstrap may create an admin key through the
  documented auth bootstrap path. Later setup calls require the admin key or a
