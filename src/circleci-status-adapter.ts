...
export class CircleCiStatusAdapter {
  ...
  declare pipelineLimit: number;
  declare jobsByWorkflowId: Map<string, any>;
...
function mergeWebhookSnapshot(previousSnapshot, payload) {
  ...
  const workflowsById = new Map<string, any>(previousSnapshot?.workflowsById ?? []);
  const jobsByWorkflowId = new Map<string, any>(previousSnapshot?.jobsByWorkflowId ?? []);
